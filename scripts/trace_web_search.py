...
from shoptech_eval import evaluate_company_with_usage_and_web_search_debug
...
def main() -> int:
    ...
    ap.add_argument("--url-column", default=os.environ.get("SHOPTECH_URL_COLUMN", "Website"), help="URL column (CSV)")
    ap.add_argument("--name-column", default=os.environ.get("SHOPTECH_NAME_COLUMN", "Name"), help="Name column (CSV)")
    ...
    ap.add_argument(
        "--max-tool-calls",
        type=int,
        default=int(os.environ.get("SHOPTECH_MAX_TOOL_CALLS") or 3),
        help="Max tool calls for the evaluation (default: env SHOPTECH_MAX_TOOL_CALLS or 3)",
    )
    ap.add_argument("--reasoning-effort", default=os.environ.get("SHOPTECH_REASONING_EFFORT") or None)
    ap.add_argument(
        "--prompt-cache",
        action="store_true",
        default=(os.environ.get("SHOPTECH_PROMPT_CACHE", "").strip() in ("1", "true", "TRUE", "yes", "YES")),
    )
    ap.add_argument("--prompt-cache-retention", default=os.environ.get("SHOPTECH_PROMPT_CACHE_RETENTION") or None)
    ap.add_argument("--service-tier", default=os.environ.get("SHOPTECH_SERVICE_TIER", "auto"))
    ap.add_argument(
        "--timeout-seconds",
        type=float,
            ...
            w.writerow(
                {
                    ...
                    "final_platform": result.get("final_platform"),
