...
BASE_SYSTEM_PROMPT = """\
You are a specialized ecommerce technology detection assistant.

You will be given:
- a shop website URL (domain)
- a rubric (below)

Your job:
- use the web search tool (this is required) to find evidence about which ecommerce platform powers the shop
- apply the rubric decision rules and evidence tiers
...
- Use the web search tool to:
  - inspect the target domain itself for direct platform markers (Tier A)
  - search the web for corroborating third-party evidence (Tier B), if needed
- Use the web search tool strategically.
  - If you have a limited tool-call/search budget, prioritize the highest-signal platform evidence first.
- Prefer first-party signals, then reputable third-party sources.
- You do NOT need to output a sources list in JSON unless explicitly requested (debug mode). Keep the output compact.

Attribution discipline:
- Be mindful of agencies, migrations, and “platform lists” that are not clearly about the provided domain.
- Prefer evidence clearly tied to the provided domain; if uncertain or conflicting, choose unknown with low confidence.
"""
    ...
    *,
    ...
    if extra_user_instructions and extra_user_instructions.strip():
        ...
        extra_instruction_block = (
            ...
            "- If (and only if) the first query does NOT yield trustworthy platform evidence for the provided domain "
            "(e.g., domain is blocked/parked, results are about other domains/entities, or evidence conflicts), "
            "you SHOULD run exactly ONE additional query focused on platform detection for that domain.\n"
            "- Do NOT use a second query just to gather extra detail when the platform is already clear.\n"
            "- Do not use more than two queries total.\n"
        )

    user_prompt = f"""\
Detect which ecommerce platform powers the shop at the provided domain.
...
Shop website URL: {normalized_url}
"""
    ...
    if prompt_cache:
        ...
        create_kwargs["prompt_cache_key"] = f"shoptech:{model}:{h}"
