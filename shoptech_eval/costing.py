...
def pricing_from_env(env: dict, default: Optional[PricingPer1M] = None) -> PricingPer1M:
    ...
    return PricingPer1M(
        input_usd=_get_float("SHOPTECH_PRICE_INPUT_PER_1M", base.input_usd),
        cached_input_usd=_get_float("SHOPTECH_PRICE_CACHED_INPUT_PER_1M", base.cached_input_usd),
        output_usd=_get_float("SHOPTECH_PRICE_OUTPUT_PER_1M", base.output_usd),
    )
...
def web_search_pricing_from_env(env: dict, default: Optional[WebSearchPricing] = None) -> WebSearchPricing:
    ...
    v = env.get("SHOPTECH_PRICE_WEB_SEARCH_PER_1K")
    if v is None or str(v).strip() == "":
        return base
