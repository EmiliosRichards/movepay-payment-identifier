...
# Shared base logic (similar to OpenAI evaluator)
BASE_SYSTEM_PROMPT = """\
...
Your job:
- research the domain using the Google Search tool (this is required) to detect which ecommerce platform powers it
- apply the rubric
...
- Ensure evidence is about the provided domain (not an agency’s other clients); if uncertain, output unknown with low confidence.
"""
...
Instructions:
- Use the Google Search tool to research the domain for direct platform markers and corroborate via reputable sources if needed.
...
- In the JSON output:
  ...
  - keep reasoning SHORT (2-4 sentences, <=600 chars).
